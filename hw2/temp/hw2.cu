...
int get_max_threadblocks(int t_per_tb) {
    ...
    // printf("max threadblocks by registers: %d\n", max_tb_by_regs);
    // printf("max threadblocks by shared memory: %d\n", max_tb_by_sharedMem);
    // printf("max threadblocks by threads per threadblock: %d\n", max_tb_by_threads);
...
// // function to initialize an consumer_additional_data struct. allocate memory for the struct and initialize the stream
// void init_consumer_additional_data(struct consumer_additional_data **cad, int nConsumers) {
//     *cad = (struct consumer_additional_data *) malloc(nConsumers * sizeof(struct consumer_additional_data));
// }
// // function for destroying an consumer_additional_data struct. free the memory of the stream and then free the struct 
// void destroy_consumer_additional_data(struct consumer_additional_data *cad, int nConsumers) {
//     free(cad);
// }

// function to init array of producer_queue structs. we want to use pinned cpu memory for them since we are going to map them 
// to the gpu using cudaHostGetDevicePointer. also since we are starting from the beginning of the queue we need to initialze
// the producer position and the consumer poistion to zero (mandatory!)
// void init_producer_queues(volatile struct producer_queue **queue, int nQueues) {
//     for (int i = 0; i < nQueues ; i++) {
//         queue[i].i_producer = 0;
//         queue[i].i_consumer = 0;
//     }
// }

// // same consideration here as in init_producer_queues
// void init_consumer_queues(volatile struct consumer_queue **queue, int nQueues) {
//     for (int i = 0; i < nQueues ; i++) {
//         queue[i].i_producer = 0;
//         queue[i].i_consumer = 0;
//     }
...
__device__ void hists2dist(int *h1, int *h2, double *distance) {
    ...
    for (int i = tid; i < HISTSIZE ; i += nThreads) {
        distance[i] = (h1[i] + h2[i] != 0) ? ((double)SQR(h1[i] - h2[i])) / (h1[i] + h2[i]) : 0;
    }
...
__global__ void consume(uchar* image1, uchar* image2, 
                        volatile bool *done,
    ...
    __shared__ volatile bool no_more_jobs, has_ready_tasks;
    __shared__ volatile int cp_sidx, img_idx;

    // there are 2 conditions for end of job. the first is that the producer should specify that there are no more tasks
    // left to be send - this is indicated by the "done flag"
    // the second condition is that this consumer should have no more tasks left to work on.
    // if both conditions are met the consumer can die.
    no_more_jobs = false; has_ready_tasks = false;
    while (!no_more_jobs || has_ready_tasks) {

        if (has_ready_tasks) {
            // the indicies are the same, but if i decied to change login i should keep them separated
            if (tid == 0) {
                img_idx = pc_queue[bid].slots[(pc_queue[bid].i_consumer) % QUEUE_SIZE].x;
                cp_sidx = (cp_queue[bid].i_consumer) % QUEUE_SIZE;
            }
            __syncthreads();
            ...
            if (tid == 0) {
                cp_queue[bid].slots[cp_sidx].x = distance[0];
                __threadfence_system();
                cp_queue[bid].i_consumer = cp_queue[bid].i_consumer + 1;
                __threadfence_system();
                pc_queue[bid].i_consumer = pc_queue[bid].i_consumer + 1;
                __threadfence_system();
            }
        }
        __syncthreads();
        ...
        if (tid == 0) {
            has_pending_tasks(&(pc_queue[bid]), (bool*)&has_ready_tasks);
            no_more_jobs = *done;
            printf("(%d %d %d) ", tid, bid, cp_queue[bid].i_producer);
        }
        __syncthreads();
    }
}
...
double collect_completed_tasks_results(volatile struct consumer_queue *queue, struct times *time, int *requests_queue) {
    double distance = 0;
    ...
    for(int i = queue->i_producer; i < last_completed_task; i++) {
        ...
        time[requests_queue[i % QUEUE_SIZE]].t_finish = get_time_msec();
    }
    // increase the producer position by the number of the completed tasks gathered
    queue->i_producer = last_completed_task;
    __sync_synchronize();

    return distance;
}

void send_task(int task_num, int img_idx, volatile struct producer_queue *queue, struct times *time, int *requests_queue) {
    ...
    requests_queue[slot_index] = task_num;
    // copy the job to the comsumer
    queue->slots[slot_index].x = img_idx;
    __sync_synchronize();
    ...
    queue->i_producer = queue->i_producer + 1;
    __sync_synchronize();
}
...
int producer_consumer_calc_distance(uchar *images1, uchar *images2, double load, struct times *time, int nConsumers, int t_per_tb) {
    ...
    volatile bool *done, *gpu_done; // flag that will indicate that there are no tasks left
    double total_distance = 0;
    ...
    CUDA_CHECK( cudaHostAlloc(&done, sizeof(bool), cudaHostAllocMapped) );
    CUDA_CHECK( cudaHostAlloc(&pc_queue, nConsumers * sizeof(struct producer_queue), cudaHostAllocMapped | cudaHostAllocWriteCombined) );
    CUDA_CHECK( cudaHostAlloc(&cp_queue, nConsumers * sizeof(struct consumer_queue), cudaHostAllocMapped | cudaHostAllocWriteCombined) );
    ...
    requests_queue = (int*) malloc(QUEUE_SIZE * nConsumers * sizeof(int));
    ...
    *done = false; // set the flag to done = false, so we indicate that there is still some work to be done
    CUDA_CHECK( cudaDeviceSynchronize() ); // sync everything with the gpu before starting
    consume<<<nConsumers, t_per_tb>>>(gpu_image1, gpu_image2, gpu_done, pc_queue, cp_queue);
    volatile int cp_i_consumer;
    volatile bool has_completed_tasks;
    while (left_requests > 0) {
        // it is important to take all the following data beforehand, to prevent from racing when we later perform the "if" checks
        cp_i_consumer = cp_queue[i_next_consumer].i_consumer;
        // if the position of the consumer is larger then the postion of the producer, then there are completed tasks
        has_completed_tasks = cp_queue[i_next_consumer].i_producer < cp_i_consumer;
        if (has_completed_tasks) {
            total_distance += 
                collect_completed_tasks_results(&(cp_queue[i_next_consumer]), time, &(requests_queue[QUEUE_SIZE * i_next_consumer]));
        }
        rate_limit_wait(&rate_limit);
        ...
        bool can_recieve_tasks = (pc_queue[i_next_consumer].i_producer - cp_queue[i_next_consumer].i_producer < QUEUE_SIZE);
        // now, if the cosumer can receive a new task - send him one
        if (can_recieve_tasks) {
            int req_num = NREQUESTS - left_requests;
            int img_idx = (req_num % N_IMG_PAIRS) * IMG_DIMENSION * IMG_DIMENSION;
            // // send task!
            printf("sending task: %d %d\n", req_num, cp_queue[i_next_consumer].i_consumer);
            send_task(req_num, img_idx, &(pc_queue[i_next_consumer]), time, &(requests_queue[QUEUE_SIZE * i_next_consumer]));
            --left_requests;
        }
        i_next_consumer = (i_next_consumer + 1) % nConsumers;
    }
    __sync_synchronize();
    *done = true; // we can tell all consumers that there are no more tasks left
    __sync_synchronize();
    CUDA_CHECK( cudaDeviceSynchronize() );
    // collect remaining results
    for (int i = 0 ; i < nConsumers; i++) {
        total_distance += collect_completed_tasks_results(&(cp_queue[i]), time, &(requests_queue[QUEUE_SIZE * i_next_consumer]));
    }
